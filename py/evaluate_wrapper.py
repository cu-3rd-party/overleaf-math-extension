"""evaluate_wrapper.py — Pyodide-compatible wrapper around lmat_cas_client's
LaTeX → SymPy → simplify → LaTeX pipeline.

Call init() once after mounting /app in Pyodide's FS, then call evaluate()
freely.  No Obsidian / TS dependencies.
"""
...
def evaluate(latex: str) -> str:
    """Compile *latex*, simplify the SymPy expression, return LaTeX string.
    ...
    ValueError
        on parse or evaluation failures (message contains detail).
    """
    if not _initialized:
